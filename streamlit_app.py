...
with my_cnx.cursor() as my_cur:
    my_cur.execute("select (count(*), sales.transaction_date) from sales where (transaction_date <= '"+d2.strftime('%Y-%m-%d')+"' and transaction_date >= '"+d1.strftime('%Y-%m-%d')+"' and local_type='Appartement')")
    header = [x[0] for x in my_cur.description]
